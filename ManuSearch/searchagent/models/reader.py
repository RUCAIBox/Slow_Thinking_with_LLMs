...
class Reader(BaseStreamingAgent):
    def __init__(self, llm:GPTAPI, webpage_cache, summary_prompt, extract_prompt, **baseconfig):
        self.llm = llm
        ...
        ##Web title:{title}
        ##Web content:{content}"""
        self.visitpage = VisitPage(api_key=search_api_key, timeout=1)
    ...
    def get_llm_summ(self, search_results:dict, question, user_query, search_intent, current_query):
        ...
        for item in cleaned_tool_return.values():
            ...
            chunked_str = chunked_str[:16192] #针对本地local模型
    ...
    def extract_text(self, tool_return):
        ...
        for item in tool_return.values():
            ...
            if len(chunked_str) > 16192:  #针对本地local模型
