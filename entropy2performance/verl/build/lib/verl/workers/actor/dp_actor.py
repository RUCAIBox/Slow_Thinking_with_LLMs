    ...
    def _optimizer_step(self,global_step):
        ...
        def simplify_name(name: str) -> str:
            ...
            for i, part in enumerate(parts):
                ...
                elif part not in {"model", "layers", "weight", "bias"} and not parts[i].isdigit():
        ...
        #     return grad_metrics 
        def get_gradient_metrics(step,save_dir="/share/project/zhipengchen/dj/RFT/grad"):
            import torch.distributed as dist
            import random
            import os
            import numpy as np
        ...
        if self.config.save_grad:
            grad_metrics=get_gradient_metrics(step=global_step)
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if self.config.use_sft_loss.use_data_from_teacher:
                        response_mask=response_mask*data["teacher_mask"]#0是teacher，1是student
                    ...
                    # print("******************************************")
                    # print("log_prob的形状",log_prob.shape)
                    # print("log_prob的第一行",log_prob[0][:200],"log_prob的第一行后面", log_prob[0][:-200])
                    # print("******************************************")
                    ####################################
                    if self.config.use_sft_loss.enabled:
                        sft_mask=data["sft_mask"]
                        print("******************************************")
                        # print("sft_mask的形状,总和",sft_mask.shape)
                        # print("attention_mask的形状",attention_mask.shape)
                        # print("log_prob的形状",log_prob.shape)
                        # print("log_prob的第一行",log_prob[0][:200],"log_prob的第一行后面", log_prob[0][:-200])
                        tmp_prob=log_prob*sft_mask
                        row_sums = tmp_prob.sum(dim=1)
                        mask = ((~torch.isnan(row_sums)) & (row_sums != 0)).float().unsqueeze(1)  # shape: [batch_size, 1]
                        ...
                        sft_loss = masked_mean(torch.nan_to_num(log_prob),sft_mask,e=True,p=True)*self.config.use_sft_loss.delta_coef
                        policy_loss=policy_loss-sft_loss
                        metrics["actor/sft_loss"] = sft_loss.detach().item()
                        print("sft loss,sft_mask=", sft_loss.detach().item(),sft_mask.sum().item())
                        print("******************************************")
                    ########################################
