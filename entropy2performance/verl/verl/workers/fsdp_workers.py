...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if self.config.actor.save_grad:
    ...
    def init_model(self):
        ...
        if self._is_ref:
            self.ref_module_fsdp = self._build_model_optimizer(model_path=self.config.ref.path,
    ...
    def generate_sequences(self,prompts: DataProto):
        ...
        with self.rollout_sharding_manager:
            ...
            output = self.rollout.generate_sequences(prompts=prompts) #use_random
            # print("#############################")
            # print(output)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_ref_log_prob(self, data: DataProto,return_negative_max_log_probs=False):
        assert self._is_ref
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            output = self.ref_policy.compute_log_prob(data=data,return_negative_max_log_probs=return_negative_max_log_probs)
            if return_negative_max_log_probs:
                output = DataProto.from_dict(tensors={'ref_negative_max_log_prob': output})
            else:
