...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1,mask_negative_adv=False,mask_teacher_output=False,mask_teacher_output_num=0,plus=1):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if mask_teacher_output:
            advantages, returns = core_algos.compute_grpo_outcome_advantage(
                token_level_rewards=token_level_rewards, eos_mask=response_mask, index=index,mask_negative_adv=mask_negative_adv,mask_teacher_output=mask_teacher_output,
        mask_teacher=data.batch["teacher_mask"],plus=plus
            )
        else:
            advantages, returns = core_algos.compute_grpo_outcome_advantage(
                token_level_rewards=token_level_rewards, eos_mask=response_mask, index=index,mask_negative_adv=mask_negative_adv
        ,plus=plus
            )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
...
class RayPPOTrainer(object):
    ...
    def __init__(
        ...
        #################
        adapt_ent_config = self.config.actor_rollout_ref.actor.adaptive_entropy #adaptive_entropy
    ...
    def _validate_config(self):
        ...
        if filter_cfg.enable:
            assert (
                filter_cfg.max_num_gen_batches > 0
            ), f"{filter_cfg.max_num_gen_batches=}"
            assert filter_cfg.metric is not None, f"{filter_cfg.metric=}"
        else:
            assert (
                config.data.train_batch_size == config.data.gen_batch_size
        ...
        if overlong_buffer_cfg.enable:
            assert (
                config.data.max_response_length >= overlong_buffer_cfg.len > 0
            ), f"{config.data.max_response_length=} / {overlong_buffer_cfg.len=}"
        else:
            assert (
                overlong_buffer_cfg.len <= 0
            ), f"{overlong_buffer_cfg.len=} > 0 but {overlong_buffer_cfg.enable=}"
            assert (
                overlong_buffer_cfg.penalty_factor <= 0
            ), f"{overlong_buffer_cfg.penalty_factor=} > 0 but {overlong_buffer_cfg.enable=}"
            assert (
                overlong_buffer_cfg.log == False
            ), f"{overlong_buffer_cfg.log=} == True but {overlong_buffer_cfg.enable=}"
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # print("val type:", type(test_gen_batch_padded))
        ...
        for data_source, prompt2var2vals in data_src2prompt2var2vals.items():
            for prompt, var2vals in prompt2var2vals.items():
                ...
                for var_name, var_vals in var2vals.items():
                    ...
                    data = [
                        {"val": val, "pred": pred} for val, pred in zip(var_vals, preds)
                    ]
                    for n in ns:
        ...
        for (
            data_source,
            var2metric2prompt_vals,
        ) in data_src2var2metric2prompt_vals.items():
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "multi_modal_inputs" in new_batch.non_tensor_batch.keys():
                    ...
                    if self.config.actor_rollout_ref.actor.use_sft_loss.use_data_from_teacher:
                        gen_batch = new_batch.pop(
                            batch_keys=["input_ids", "attention_mask", "position_ids"],
                            non_tensor_batch_keys=["raw_prompt_ids","teacher_rollout_text"],
                        )
                    else:
                        gen_batch = new_batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.actor_rollout_ref.actor.use_sft_loss.use_data_from_teacher:
                        new_batch = new_batch.repeat(
                            repeat_times=self.config.actor_rollout_ref.rollout.n+ self.config.actor_rollout_ref.rollout.add_teacher_rollout.n,
                            interleave=True,
                        )
                    else:
                        new_batch = new_batch.repeat(
                    ...
                    new_batch = new_batch.union(gen_batch_output)#get train data
                    with _timer("reward", timing_raw):
                        ...
                        solve_none = 0
                        solve_all = 0
                        mask = np.zeros_like(np.array(new_batch.non_tensor_batch["acc"]))
                        for uid in unique_uids:
                            uid_mask = uids == uid
                            if self.config.actor_rollout_ref.actor.use_sft_loss.use_data_from_teacher:
                                uid_rewards=np.array(new_batch.non_tensor_batch["acc"])[uid_mask][:-self.config.actor_rollout_ref.rollout.add_teacher_rollout.n]
                            else:
                                uid_rewards=np.array(new_batch.non_tensor_batch["acc"])[uid_mask]
                            # Check if all rewards are 0 or all are 1 for this uid
                            ###################################################
                            if self.config.actor_rollout_ref.actor.use_sft_loss.enabled is True:
                                tmp=np.mean(uid_rewards)
                                if tmp>=self.config.actor_rollout_ref.actor.use_sft_loss.lower_acc_bound and tmp<=self.config.actor_rollout_ref.actor.use_sft_loss.upper_acc_bound:
                                    mask[uid_mask]=1 
                            ###################################################
                            if self.config.algorithm.filter_groups.bound_filter_groups:
                                tmp=np.mean(uid_rewards)
                                if tmp <=self.config.algorithm.filter_groups.filter_groups_low:
                                    solve_none += 1
                                elif tmp>=self.config.algorithm.filter_groups.filter_groups_high:
                                    solve_all+=1
                        ...
                        if self.config.actor_rollout_ref.actor.use_sft_loss.enabled:
                            # mask = torch.tensor(np.expand_dims(mask, axis=-1), dtype=torch.float32)
                            mask=torch.tensor(mask).unsqueeze(1) 
                            acc_tensor=torch.tensor(new_batch.non_tensor_batch["acc"]).unsqueeze(1) 
                            print("*************************************")
                            print("sft_mask:", new_batch.batch["sft_mask"].sum().item())
                            print("满足acc限制的数据总和:", mask.sum().item())
                            print("正确数据总和:", acc_tensor.sum().item())
                            new_batch.batch["sft_mask"]=new_batch.batch["sft_mask"]*mask*acc_tensor
                            print("更新后 sft_mask 总和:", new_batch.batch["sft_mask"].sum().item())
                            # print(new_batch.batch["sft_mask"].shape)
                            print("**************************************")
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        if metric_name == "seq_final_reward":
                            ...
                            new_batch.non_tensor_batch["seq_reward"] = (
                                ...
                                .sum(dim=-1)
                                .numpy()
                            ) #seq_reward和seq_final_reward有啥区别，这俩不一样的吗？

                        # Collect the sequence reward for each trajectory
                        prompt_uid2metric_vals = defaultdict(list)

                        if not self.config.actor_rollout_ref.actor.use_sft_loss.use_data_from_teacher:
                            ...
                            for uid, metric_val,teacher_mask in zip(
                                new_batch.non_tensor_batch["uid"],
                                new_batch.non_tensor_batch[metric_name],
                                new_batch.batch["teacher_mask"]
                            ):
                                if teacher_mask.item()!= 0:
                                    prompt_uid2metric_vals[uid].append(metric_val)
                            # for 
                            # if self.config.actor_rollout_ref.actor.use_sft_loss.use_data_from_teacher:
                            #     for uid, metric_vals in prompt_uid2metric_vals.items():
                            #         metric_vals = metric_vals[:-self.config.rollout.add_teacher_rollout.n]
                            #         prompt_uid2metric_vals[uid] = metric_vals


                        # print(f"{prompt_uid2metric_vals=}")

                        prompt_uid2metric_std = {}
                        if self.config.algorithm.filter_groups.bound_filter_groups: 
                            for prompt_uid, metric_vals in prompt_uid2metric_vals.items():
                                prompt_uid2metric_std[prompt_uid] = np.sum(metric_vals)/len(metric_vals)
                            # print(f"{prompt_uid2metric_std=}")

                            kept_prompt_uids = [
                                uid for uid, std in prompt_uid2metric_std.items() if std > self.config.algorithm.filter_groups.filter_groups_low and self.config.algorithm.filter_groups.filter_groups_high>std
                            ]
                        else:
                            for prompt_uid, metric_vals in prompt_uid2metric_vals.items():
                        ...
                        for idx, traj_from_prompt_uid in enumerate(
                            ...
                            if traj_from_prompt_uid in kept_prompt_uids:
                                ...
                                not_use_traj_idx.append(idx)
                        if self.config.algorithm.save_all_data:
                            if save_batch is None:
                                save_batch={}
                                save_batch["used"]=new_batch[kept_traj_idxs]
                                save_batch["not_use"]=new_batch[not_use_traj_idx]
                            else:
                                save_batch["used"]=DataProto.concat([new_batch[kept_traj_idxs], save_batch["used"]])
                                save_batch["not_use"]=DataProto.concat([new_batch[not_use_traj_idx], save_batch["not_use"]])

                        new_batch = new_batch[kept_traj_idxs]
                        # print("**************************************************")
                        # print(new_batch.batch.keys())
                        # print(new_batch.non_tensor_batch.keys())
                        # print(new_batch.meta_info.keys())
                        # print(new_batch.aa.keys())
                        # print("**************************************************")
                        if batch is None:
                            batch = new_batch
                        else:
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            if self.config.algorithm.save_all_data:
                                if save_batch is None:
                                    save_batch["not_use"]=batch[traj_bsz:]
                                else:
                                    save_batch["not_use"]=DataProto.concat([batch[traj_bsz:], save_batch["not_use"]])
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            mask_negative_adv=self.config.algorithm.mask_negative_adv,
                            mask_teacher_output=self.config.actor_rollout_ref.actor.use_sft_loss.use_data_from_teacher,
                            mask_teacher_output_num=self.config.actor_rollout_ref.rollout.add_teacher_rollout.n
                        )
                    # update critic
                    if self.use_critic:
                ...
                # collect metrics
                if self.adapt_ent_config.enabled:
                    ...
                    if data_dict:
                        for key, data in data_dict.items():
                            for i in range(len(data)):
                                ...
                                acc=data_item.non_tensor_batch["acc"]
                                if key in save_dict:
                                    save_dict[key].append({"problem":prompt_str,"output":response_str,"ground_truth":ground_truth,"data_source":data_source,"acc":acc,"state":key})
                                else:
                                    save_dict[key]=[{"problem":prompt_str,"output":response_str,"ground_truth":ground_truth,"data_source":data_source,"acc":acc,"state":key,"length":length}]
                        data_path = f"{self.config.trainer.default_local_dir}/rollout_data_step{self.global_steps}.pkl"
                        # import json
                        ...
                        total_items = sum(len(v) for v in save_dict.values())
                        print(f"已保存 {total_items} 条数据到 {data_path}")
                if self.config.algorithm.save_all_data:
                    process_data(save_batch)
                    save_batch=None
                num_prompt_in_batch = 0
                num_gen_batches = 0
