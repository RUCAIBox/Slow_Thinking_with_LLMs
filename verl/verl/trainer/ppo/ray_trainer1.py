    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        if last_valid_batch is not None:
                            batch = DataProto.concat([last_valid_batch, new_batch])
                            last_valid_batch = None
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            if (
                                ...
                                dynamic_batch_size = max(
                                    1, dynamic_batch_size
                                )  # 确保batch size至少为1
                                self.train_dataloader.batch_size = dynamic_batch_size
                                self.train_dataloader.batch_sampler.batch_size = (
                                    dynamic_batch_size
                                )
                                print(
                                    f"Dynamically adjusted batch_size to {dynamic_batch_size}"
                                )
                                continue
                            else:
                                raise ValueError(
                            ...
                            if len(batch) > traj_bsz:
                                last_valid_batch = batch[traj_bsz:]
                                print(
                                    f"Saved {len(last_valid_batch)} valid trajectories for next round"
                                )
                            batch = batch[:traj_bsz]
                            # 重置dataloader的batch size为默认值
                            if (
                                hasattr(self.train_dataloader, "batch_size")
                                and self.train_dataloader.batch_size
                                != self.config.data.train_batch_size
                            ):
                                ...
                                print(
                                    f"Reset batch_size to {self.config.data.train_batch_size}"
                                )
                ...
                if last_valid_batch is not None:
                    saved_prompt_count = (
                        len(last_valid_batch) // self.config.actor_rollout_ref.rollout.n
                    )
                    dynamic_batch_size = (
                        self.config.data.train_batch_size - saved_prompt_count
                    ) * 2
                    dynamic_batch_size = max(
                        1, dynamic_batch_size
                    )  # 确保batch size至少为1
                    self.train_dataloader.batch_size = dynamic_batch_size
                    self.train_dataloader.batch_sampler.batch_size = dynamic_batch_size
                    print(
                        f"End of step: Dynamically adjusted next batch_size to {dynamic_batch_size} based on {saved_prompt_count} saved prompts"
                    )
