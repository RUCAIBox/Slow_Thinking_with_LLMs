...
def compute_advantage(
    ...
    lam=1.0,
    num_repeat=1,
    mask_negative_adv=False,
    mask_teacher_output=False,
    mask_teacher_output_num=0,
    plus=1,
):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if mask_teacher_output:
            advantages, returns = core_algos.compute_grpo_outcome_advantage(
                ...
                eos_mask=response_mask,
                index=index,
                mask_negative_adv=mask_negative_adv,
                mask_teacher_output=mask_teacher_output,
                mask_teacher=data.batch["teacher_mask"],
                plus=plus,
            )
        else:
            advantages, returns = core_algos.compute_grpo_outcome_advantage(
...
class RayPPOTrainer(object):
    ...
    def __init__(
        ...
        self.use_reference_policy = (
            self.config.actor_rollout_ref.actor.use_ref
        ...
        #################
        adapt_ent_config = (
            self.config.actor_rollout_ref.actor.adaptive_entropy
        )  # adaptive_entropy
        use_adapt_ent = (
            adapt_ent_config.enabled
            or self.config.actor_rollout_ref.actor.use_dynamic_clip.enabled
        )
        self.ent_ctrl = core_algos.EntController(
            init_ent_coef=self.config.actor_rollout_ref.actor.entropy_coeff,
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "multi_modal_inputs" in new_batch.non_tensor_batch.keys():
                    ...
                    if (
                        self.config.actor_rollout_ref.actor.use_sft_loss.use_data_from_teacher
                    ):
                        gen_batch = new_batch.pop(
                            batch_keys=["input_ids", "attention_mask", "position_ids"],
                            non_tensor_batch_keys=[
                                "raw_prompt_ids",
                                "teacher_rollout_text",
                            ],
                        )
                    else:
                        gen_batch = new_batch.pop(
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        # if self.config.actor_rollout_ref.rollout.use_random:
                        #     config_path = "/share/project/zhipengchen/dj/RFT/code/rl/config.json"
                        #     import json
                        #     with open(config_path, "r") as f:
                        #         tmp_config = json.load(f)
                        #     if tmp_config["random_roll"] is False:
                        #         tmp_config["random_roll"] = True
                        #         with open(config_path, "w") as f:
                        #             json.dump(tmp_config, f, indent=2)
                        gen_batch_output = self.actor_rollout_wg.generate_sequences(
                            prompts=gen_batch
                        )  # self.config.actor_rollout_ref.rollout.use_random
                    ...
                    if (
                        ...
                        new_batch = new_batch.repeat(
                            ...
                            + self.config.actor_rollout_ref.rollout.add_teacher_rollout.n,
                            interleave=True,
                        )
                    else:
                        new_batch = new_batch.repeat(
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        solve_all = 0
                        mask = np.zeros_like(
                            np.array(new_batch.non_tensor_batch["acc"])
                        )
                        for uid in unique_uids:
                            ...
                            ###################################################
                            if (
                                self.config.actor_rollout_ref.actor.use_sft_loss.enabled
                                is True
                            ):
                                tmp = np.mean(uid_rewards)
                                if (
                                    tmp
                                    >= self.config.actor_rollout_ref.actor.use_sft_loss.lower_acc_bound
                                    and tmp
                                    <= self.config.actor_rollout_ref.actor.use_sft_loss.upper_acc_bound
                                ):
                                    mask[uid_mask] = 1
                            ###################################################
                            if self.config.algorithm.filter_groups.bound_filter_groups:
                                ...
                                if (
                                    tmp
                                    <= self.config.algorithm.filter_groups.filter_groups_low
                                ):
                                    solve_none += 1
                                elif (
                                    tmp
                                    >= self.config.algorithm.filter_groups.filter_groups_high
                                ):
                                    solve_all += 1
                        ...
                        if self.config.actor_rollout_ref.actor.use_sft_loss.enabled:
                            ...
                            wrong_tensor = ~acc_tensor
                            print("*************************************")
                            print("sft_mask:", new_batch.batch["sft_mask"].sum().item())
                            print("满足acc限制的数据总和:", mask.sum().item())
                            # print("正确数据总和:", acc_tensor.sum().item())
                            new_batch.batch["sft_mask"] = (
                                new_batch.batch["sft_mask"] * mask * wrong_tensor
                            )
                            print(
                                "更新后 sft_mask 总和:",
                                new_batch.batch["sft_mask"].sum().item(),
                            )
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        if (
                            not self.config.actor_rollout_ref.actor.use_sft_loss.use_data_from_teacher
                        ):
                        ...
                        if self.config.algorithm.filter_groups.bound_filter_groups:
                            for (
                                prompt_uid,
                                metric_vals,
                            ) in prompt_uid2metric_vals.items():
                                prompt_uid2metric_std[prompt_uid] = np.sum(
                                    metric_vals
                                ) / len(metric_vals)
                            ...
                            kept_prompt_uids = [
                                ...
                                if std
                                > self.config.algorithm.filter_groups.filter_groups_low
                                and self.config.algorithm.filter_groups.filter_groups_high
                                > std
                            ]
                        else:
                            for (
                        ...
                        if self.config.algorithm.save_all_data:
                            if save_batch is None:
                                ...
                                save_batch["used"] = DataProto.concat(
                                    [new_batch[kept_traj_idxs], save_batch["used"]]
                                )
                                save_batch["not_use"] = DataProto.concat(
                                    [new_batch[not_use_traj_idx], save_batch["not_use"]]
                                )
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            if self.config.algorithm.save_all_data:
                                if save_batch is None:
                                    ...
                                    save_batch["not_use"] = DataProto.concat(
                                        [batch[traj_bsz:], save_batch["not_use"]]
                                    )
                            if self.config.actor_rollout_ref.rollout.use_random:
                                config_path = "/share/project/zhipengchen/dj/RFT/code/rl/config.json"
                                with open(config_path, "r") as f:
                                    tmp_config = json.load(f)
                                if tmp_config["random_roll"] is True:
                                    tmp_config["random_roll"] = False
                                    with open(config_path, "w") as f:
                                        json.dump(tmp_config, f, indent=2)
                    ...
                    if self.use_reference_policy:
                        ...
                        with _timer("ref", timing_raw):
                            ...
                            # if self.xx.punish_latent_negative_path:
                            #     max_negative_log_prob=self.ref_policy_wg.compute_ref_log_prob(data=batch,return_negative_max_log_probs=True)
                            #     batch = batch.union(max_negative_log_prob)
                        # print(batch)
                        # print(ref_log_prob)
                    # compute values
                    if self.use_critic:
                    ...
                    if self.config.actor_rollout_ref.actor.use_ppl:
                        batch = core_algos.calculate_ppl(batch)
                    # update critic
                    if self.use_critic:
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer("update_actor", timing_raw):
                            ...
                            if self.global_steps<300 and self.global_steps>200:
                                batch.meta_info["use_location"]=True
                            else:
                                batch.meta_info["use_location"]=False
                    ...
                    if data_dict:
                        for key, data in data_dict.items():
                            for i in range(len(data)):
                                ...
                                valid_response_length = data_item.batch[
                                    "attention_mask"
                                ][prompt_length:].sum()
                                ...
                                ground_truth = data_item.non_tensor_batch[
                                    "reward_model"
                                ]["ground_truth"]
